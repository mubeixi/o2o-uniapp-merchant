...
const upFileFnByAliyunOss = async ({
  file = {}, current_path, name = 'file', idx, list = [], progress,
}) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.upload.addEventListener('progress', (evt) => {
      ...
      if (evt.lengthComputable) {
        const percentComplete = parseInt(evt.loaded / evt.total * 100 * 100) / 100;
...
const upFileFnByLocal = async ({
  file = {}, current_path, name = 'image', idx, list = [], progress, extParam = {},
...
export default class WzwFileButton extends Vue {
        ...
        fileFn(e) {
          ...
          for (let i = 0; i < curFiles.length; i++) {
            const curFileSize = parseInt(curFiles[i].size / 1024 * 100) / 100;
