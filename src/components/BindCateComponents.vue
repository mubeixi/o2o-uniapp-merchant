...
import { deepCopy, get_arr_column, objTranslate } from '@/common/utils';
          ...
          if (!this.innerDialog.classify.isHasData) {
            getProductCategory()
              .then((res) => {
                ...
                const orogin_cate_list = objTranslate(res.data);
                ...
                const ids = get_arr_column(this.has, 'Category_ID');

                for (const cate of orogin_cate_list) {
            ...
            const { orogin_cate_list } = this.innerDialog.classify;
      ...
      if (this.innerDialog.index === 'customize') {
        ...
        switch (this.innerDialog.customizeIndex) {
          ...
          case '2':
            ...
            if (this.multiple) {
              const arr = this.$refs.treeForm.getCheckedNodes();
              console.log(arr);
              if (arr.length < 1) return this.$message('请先选择分类');
              const nameArr = arr.map(item => item.label);
              ...
              const data = this.$refs.treeForm.getCheckedNodes()[0];
