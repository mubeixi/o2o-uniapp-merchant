...
import {
  objTranslate, GetQueryByString, deepCopy, getStyle, trim,
} from '@/common/utils';
    ...
    @Component({
      ...
      methods: {
        ...
        async uploadConfig(is_use, pre, call) {
          ...
          // 不是diy
          if (!this.isDiy) {
            const Skin_ID = parseInt(ss.get('Skin_ID'));
            const Home_ID = parseInt(ss.get('Home_ID'));
            if (!Skin_ID && !Home_ID) {
...
export default class PreviewComponent extends Vue {
        ...
        created() {
          const _self = this;
          new Promise((resolve, reject) => {
            const postData = {}; let getSkinConfigFunc = null;
            // 普通模式
            if (!_self.isDiy) {
              const Skin_ID = parseInt(ss.get('Skin_ID'));
              const Home_ID = parseInt(ss.get('Home_ID'));
