...
<!--    -->
    <el-form ref="form" :model="form"  :label-width="activeAttr.attrData.labelSize==='L'?'100px':'80px'">
        <template v-for="(item, index) in activeAttr.attrData.content">
...
import { FUNFinder } from './editor/ckeditt-resource/FUNFinder';
    ...
    @Component({
      ...
      methods: {
        ...
        openGoodsBindCate(item, success, idx2, bindCateMultiple, has_cate_list) {
          ...
          // 居然还要自己拼接数据格式，醉哦
          this.has_cate_list = has_cate_list.map(itemVal => ({ Category_ID: itemVal }));

          // 先改好数据再搞事
          const self = this;
          setTimeout(() => {
            self.bindCateDialogShow = true;
          }, 50);
        },
