<template>
  <view @click="commonClick" v-if="loading">
    ...
    <view @click="goAddressList" class="address" v-if="giftInfo.Gift_Shipping== 1">
      <image src="/static/location.png" alt="" class="loc_icon"></image>
      <block v-if="have_Order_ID">
        <view class="add_msg">
          <view class="name">收货人：{{initDataValue.Address_Name}}
            <span>{{initDataValue.Address_Mobile }}</span>
          </view>
          <view class="location">
      ...
      <block v-else>
        <view class="add_msg" v-if="addressinfo.Address_Name">
          <view class="name">收货人：{{addressinfo.Address_Name}} <span>{{addressinfo.Address_Mobile }}</span>
          </view>
          <view class="location">
