<template>
    <div class="goods wrap" id="goods"
        ...
        <div :class="className" class="listbox">
            <ul class="list">
                <li @click="goProductDetail(item.Products_ID)" v-for="(item,idx) in goodsList"
                    :key="idx" class="item"
...
<script>
    ...
    import {createEmptyArray, getCountdownFunc} from "../../common/helper";
    ...
    export default {
        ...
        methods: {
            ...
            goProductDetail(id){
                if (!id) return;
                let path = '/pages/detail/groupDetail'
                uni.navigateTo({
