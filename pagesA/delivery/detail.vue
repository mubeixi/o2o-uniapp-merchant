...
<template>
  <div class="page-wrap"  @click="commonClick">
    ...
    <scroll-view
      @scrolltoupper="bindScrollToupperEvent"
      @scrolltolower="bindScrollLowerEvent"
      @scroll="bindScrollEvent"
      @touchmove.stop.prevent="bindScrollTouchEvent"
      ...
      <div id="section0">
        ...
        <block  v-show="isReady">
          ...
          <div class="product-price fz-14">
            ...
            <div class="product-price-right" @click="addCart">
              加入购物车
            </div>
          </div>
          ...
          <div class="carts-item" v-for="(row,idx) in carts" :key="idx">
            ...
            <div class="carts-item-info">
              ...
              <div class="actions">
                ...
                <div style="width: 150rpx" class="action flex flex-vertical-c">
                  <block v-if="row.num>0">
                    <layout-icon @click.stop="attrNumMinus(row,idx)" size="24" color="#26C78D" type="iconicon-minus"></layout-icon>
                    <input @input="changeAttrNum($event,idx)" :value="row.num" class="input-num text-center fz-12" />
                  </block>
                  <layout-icon @click.stop="attrNumPlus(row,idx)" size="24" color="#26C78D" type="iconicon-plus"></layout-icon>
                </div>
              </div>
            </div>
          </div>
