<template>
  <div  class="wrap">
        ...
        <div class="pro" v-for="(item,index) of refundInfo.refund_prod_list" :key="item">
            ...
            <div class="pro-msg">
				...
				<div class="fz-14  graytext2  c9">
					<div>退回余额:<span class="fz-16 danger-color"><span class="span fz-12">￥</span>{{item.refund_money_fee}} </span></div>
					<div>原路退回:<span class="fz-16 danger-color"><span class="span fz-12">￥</span>{{item.refund_pay_fee}} </span></div>
				</div>
            </div>
        </div>
        ...
        <div class="item " @click="showReason">
            ...
            <div class="item-right flex flex-vertical-c" >
						<span>{{reasonDes?reasonDes: '请选择'}}</span>
							<layout-icon class="img" type="iconicon-arrow-right" size="16"></layout-icon>
            </div>
        </div>
        <div class="item spe">
        ...
        <div class="imgs">
			<view class="shangchuans" v-for="(item,index) of imgs" :key="index"  >
				...
				<layout-icon class="image del" type="iconicon-test" size="20"  @click="delImg(index)"></layout-icon>
		...
		<popup-layer ref="popup" :direction="'top'" style="z-index: 99;">
			<div class="bMbx">
