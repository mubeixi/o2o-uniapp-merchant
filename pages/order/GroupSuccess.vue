  ...
  methods: {
    ...
    async _init_func () {
      try {
        showLoading()
        // 这里要设置Prod_ID
        const orderInfo = await getOrderDetail({ Order_ID: this.Order_ID }, { onlyData: 1 }).catch((e) => { throw Error(e.msg || '获取订单详情失败') })
        ...
        const productInfo = await getProductDetail({ prod_id: this.Prod_ID, teamid: this.Team_ID }, { onlyData: 1 }).catch((e) => { throw Error(e.msg || '获取产品详情失败') })
