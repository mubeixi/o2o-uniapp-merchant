	...
	<div class="wrap">
		...
		<div class="functions flex flex-justify-between">
			<block v-for="item in funList">
				<LayoutFun :type="item.className" :name="item.name" :color="item.color"></LayoutFun>
			</block>
		</div>
		<div class="intro">为你推荐</div>
		<div class="product-list flex">
			<block v-for="item in proList">
				<ProTag :pro_src="item.src" :pro_name="item.name" :pro_price="item.price" :pro_price_old="item.old_price"></ProTag>
			</block>
		</div>
	</div>
</template>
...
export default {
  components: {
    ...
    return {
      funList: [
	      {className: 'iconpintuan',name: '拼团订单', color:'#FE7602'},
	      {className: 'iconxianshiqianggou',name: '限时折扣单', color:'#ADCF81'},
	      {className: 'iconmiaosha',name: '秒杀订单', color:'#54AEED'},
	      {className: 'iconzengpin',name: '我的赠品', color:'#E8779F'},
	      {className: 'iconyue',name: '余额', color:'#69C276'},
	      {className: 'iconjifen',name: '积分', color:'#52CCCD'},
	      {className: 'iconshoucang18-copy',name: '收藏', color:'#FF7F79'},
	      {className: 'iconyouhuiquan',name: '优惠券', color:'#D1BE71'},
	      {className: 'iconrenwu',name: '任务中心', color:'#54AEED'},
	      {className: 'icondizhi',name: '地址管理', color:'#E8779F'},
	      {className: 'icontuikuan',name: '退款/售后', color:'#FE7602'},
	      {className: 'iconfenxiao',name: '分销中心', color:'#69C276'}
      ],
	    proList: [
		    {
