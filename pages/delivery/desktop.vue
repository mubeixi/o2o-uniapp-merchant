<template>
  <div @click="commonClick" class="page-wrap">
    ...
    <div :style="{height:menuButtonInfo.top+menuButtonInfo.height+'px'}"
      ...
      <!--导航栏-->
      <div :style="{height:menuButtonInfo.height+'px',backgroundColor:menuButtonBgColor}" class="navigation-bar">
        <layout-icon @click="$back" class="left-icon" color="#fff" size="18"
                     type="iconicon-arrow-left"></layout-icon>
        <div :style="{lineHeight:menuButtonInfo.height+'px'}" class="title color-white">外卖</div>
      </div>
    </div>
    <div :style="{backgroundImage: 'url('+$getDomain('/static/client/delivery/desktop_top_bg.jpg')+')'}" class="head">
      ...
      <div class="store-info flex-vertical-c flex flex-justify-c">
        <image :src="bizInfo.biz_logo" class="store-logo"></image>
          ...
          <div :key="idx" class="carts-item" v-for="(row,idx) in carts">
            ...
            <div class="carts-item-info">
              ...
              <div class="actions">
                ...
                <div @click="setActiveAttrIdx(idx)" class="action flex flex-vertical-c" style="width: 120px">
                  <block v-if="row.num>0">
                    <layout-icon @click.stop="attrNumMinus(row)" color="#26C78D" size="24"
                                 type="iconicon-minus p-10"></layout-icon>
                    <input :value="row.num" @input="changeAttrNum" class="input-num text-center fz-12" />
                  </block>
                  <layout-icon @click.stop="attrNumPlus(row)" color="#26C78D" size="24"
                               type="iconicon-plus p-10"></layout-icon>
                </div>
              </div>
            </div>
